...
module.exports = async function({ components, cwd }) {
  const componentsFile = require.resolve(components, {
    paths: [cwd]
  });
  ...
  const defaultInclude = [path.dirname(componentsFile)];
  ...
  console.log(
    declarations[
      'examples/seek-style-guide/node_modules/seek-style-guide/react/Alert/Alert.d.ts'
    ]
  );
